...
import { ChevronLeft, Copy, CheckCircle, Calculator } from 'lucide-react';
...
import { generateRegexString, generateBlocksForNumbers } from '@/components/regex-vision/utils';
...
export default function ExtractNumbersResultPage() {
  const router = useRouter();
  const { toast } = useToast();
  const [generatedRegex, setGeneratedRegex] = useState<string>('');

  useEffect(() => {
    const numbersBlocks: Block[] = generateBlocksForNumbers();
    const regexString = generateRegexString(numbersBlocks);
  ...
  const handleCopyToClipboard = () => {
    if (generatedRegex) {
      navigator.clipboard.writeText(`/${generatedRegex}/g`) // Assuming global flag for extraction
        .then(() => toast({ title: "Скопировано!", description: "Регулярное выражение для извлечения чисел скопировано." }))
  ...
  const handleGoToEditor = () => {
    if (generatedRegex) {
        ...
        .then(() => toast({ title: "Скопировано!", description: "Regex для извлечения чисел скопирован. Переход в редактор..." }))
  ...
  return (
    <div className={`${inter.variable} ${jetbrainsMono.variable} wizard-body-styles wizard-step-page`}>
      <Head>
        <title>Мастер: Извлечение Чисел - Результат</title>
      </Head>
      ...
      <div className="wizard-header">
        ...
        <h1>Извлечение: Все числа - Готовое выражение</h1>
        <p>Ниже представлено регулярное выражение для поиска и извлечения чисел (целых, десятичных, положительных, отрицательных).</p>
      </div>
      ...
      <Card className="w-full max-w-lg mx-auto">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Regex для извлечения чисел
          </CardTitle>
          <CardDescription>
            Это выражение находит числа, включая целые, десятичные, со знаком и без. Число захватывается в группу.
