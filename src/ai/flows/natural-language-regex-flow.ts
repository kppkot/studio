...
2.  "settings": An object with settings specific to the type. Examples:
    ...
    *   For "CHARACTER_CLASS": Use the 'pattern' property to define the characters. For simple shorthands like \`\\d\` or \`\\w\`, use them directly in the pattern. For custom sets, combine them, for example: \`a-zA-Z0-9_\`. For a regex like \`[A-Za-z0-9._%+-]\`, you should create ONE block: \`{ "type": "CHARACTER_CLASS", "settings": { "pattern": "A-Za-z0-9._%+-", "negated": false } }\`.
