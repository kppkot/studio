...
1.  **VALIDITY ACROSS ALL EXAMPLES IS PARAMOUNT:** Your proposed step **must** be valid for ALL positive examples in the test text. This is the most important rule. If a component (like a separator) is present in some examples (e.g., "PO-123") but absent in others (e.g., "PO123"), that component **MUST be made optional**. The standard way to do this is to create the component block in one step, and then create a \`QUANTIFIER\` block with type \`?\` in the very next step. Do not propose a mandatory component if it is not present in every single example.
