...
4.  **ANALYZE ALL EXAMPLES FOR OPTIONALITY:** Before proposing a step, look at ALL positive examples in the \`exampleTestText\`. Your next logical step must be valid for ALL of them. If you see that a character or sequence (like a separator) is present in some examples (e.g., "PO-123") but absent in others (e.g., "PO123"), you **MUST** first generate the block for that character/sequence, and then in the **immediately following step**, you MUST make that part optional by generating a \`QUANTIFIER\` block with settings \`{"type": "?"}\`.
